...
// short-circuit emitted by client.go.tmpl. Under BERKELEY_CLASSES_VERIFY=1
...
func TestClient_VerifyShortCircuit_LiveHTTPOptIn(t *testing.T) {
	t.Setenv("BERKELEY_CLASSES_VERIFY", "1")
	t.Setenv("BERKELEY_CLASSES_VERIFY_LIVE_HTTP", "1")
	c, rec := newClientWithRecorder(t)
...
func TestClient_VerifyShortCircuit_NoEnv(t *testing.T) {
	...
	t.Setenv("BERKELEY_CLASSES_VERIFY", "")
	t.Setenv("BERKELEY_CLASSES_VERIFY_LIVE_HTTP", "")
	c, rec := newClientWithRecorder(t)
...
// BERKELEY_CLASSES_VERIFY=1, because they cannot mutate remote state.
...
// under BERKELEY_CLASSES_VERIFY=1, because the operation does not mutate
